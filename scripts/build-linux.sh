...
step3_patch_bundle() {
  ...
  if [[ -n "$target_bundle" ]]; then
    ...
    # Disable the status-pill drag-to-reposition gesture on Linux: absolute
    # window positioning by the client has no equivalent under native Wayland,
    # so the drag can never complete, and the input-blocking dimming overlay it
    # raises can strand itself over part of the screen. See
    # docs/learnings/platform-gates.md and linux-disable-pill-drag.sh's header.
    auto "Running linux-disable-pill-drag.sh on $target_bundle"
    bash "$SCRIPT_DIR/patches/linux-disable-pill-drag.sh" "$target_bundle" \
      || warn "Pill-drag patch failed -- see linux-disable-pill-drag.sh output above."
    # Shrink the status-pill window from 440x320 to the pill's own footprint;
    # the invisible transparent canvas swallows clicks on Wayland.
    auto "Running linux-status-compact.sh on $target_bundle"
    bash "$SCRIPT_DIR/patches/linux-status-compact.sh" "$target_bundle" \
      || warn "Status-compact patch failed -- see linux-status-compact.sh output above."
    # Implement Electron's login-item API on Linux (XDG autostart, launched
    # --hidden so a login start stays in the tray).
    ...
    # Wayland reports window.screenX/Y as 0, so pickers the pill opens (language,
    # auto-polish, fetch) anchor at the screen's left edge; give the status
    # renderer its real bottom-centred position instead.
