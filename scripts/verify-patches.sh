...
#     * linux-disable-pill-drag.sh -> disables the status-pill drag gesture,
#       which can never complete under native Wayland
#     * linux-status-compact.sh -> status window sized to the pill on Linux
